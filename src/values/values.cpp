...
ArrayNumberValue *getValue(string identifier, long index) {
    if (getSymbolTable()->variableDeclared(identifier)) {
        ...
        if (variable->varType == ARRAY) {
            ...
            if (index >= array->startId && index <= array->startId + array->length - 1) {
                ...
                error("Nieprawidłowy indeks '" + to_string(index) + "' dla tablicy '" + identifier + "'");
            }
        } else {
        ...
        if (variable->varType == ARRAY) {
            ...
            if (indexValue) {
                ...
                if (indexVariable->initialized) {
                    ...
                    error("Użycie niezainicjalizowanej zmiennej '" + indexIdentifier + "'");
                }
            }
        } else {
...
void valueNotInitialized(Value *value) { error("Użycie niezainicjalizowanej zmiennej " + value->toString()); }
...
vector<string> dynamicArrayPositionToRegister(ArrayVariable *array, ValueVariable *index, Register reg) {
    ...
    vector<string> genArrayMem = generateNumberInRegister(array->memoryId, Registers::E);
    vector<string> genArrayStart = generateNumberInRegister(array->startId, Registers::F);
...
vector<string> valueAdressToRegister(Value *value, Register reg) {
    ...
    switch (value->type) {
        case NUMBER: {
            ...
            auto genInstructions = tempConstToRegister(numberValue->number, reg);
        ...
        case POINTER: {
            auto pointerValue = (PointerValue *)value;
            auto genInstructions = generateNumberInRegister(pointerValue->variable->memoryId, reg);
        ...
        case ARRAY_NUMBER: {
            ...
            auto genInstructions =
                generateNumberInRegister(arrayNumberValue->array->getMemoryId(arrayNumberValue->index), reg);
        ...
        case ARRAY_POINTER: {
            ...
            auto genInstructions =
                dynamicArrayPositionToRegister(arrayPointerValue->array, arrayPointerValue->indexVariable, reg);
            concatStringsVectors(&instructions, &genInstructions);
            break;
        }
        default:
...
vector<string> valueToRegister(Value *value, Register reg) {
    ...
    switch (value->type) {
        case NUMBER: {
            auto numberValue = (NumberValue *)value;
            auto genInstructions = generateNumberInRegister(numberValue->number, reg);
            concatStringsVectors(&instructions, &genInstructions);
            break;
        }
        default: {
